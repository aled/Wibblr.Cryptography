...
namespace Wibblr.Cryptography
{
    ...
    public static class Sha2Crypt
    {
        ...
        public static bool Validate(string key, string hash)
        {
            ...
            throw new ArgumentException("No SHA256 or SHA512 algorithm identifier in hash", nameof(hash));
        }
        ...
        private static string Crypt(string algorithm, byte[] keyBytes, string salt = "")
        {
            ...
            {
                var parts = salt.Split(new[] { SALT_FIELD_SEPARATOR }, StringSplitOptions.RemoveEmptyEntries);
                int i = 0;
        ...
        private static string Crypt(byte[] keyBytes, int rounds, string saltString, bool roundsCustom, string algorithm)
        {
            ...
            ArrayUtils.Fill(tempResult, (byte)0);
            ArrayUtils.Fill(pBytes, (byte)0);
            ArrayUtils.Fill(sBytes, (byte)0);
            ctx.Clear();
            altCtx.Clear();
            ArrayUtils.Fill(keyBytes, (byte)0);
            ArrayUtils.Fill(saltBytes, (byte)0);
